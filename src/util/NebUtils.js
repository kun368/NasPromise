...
const dappAddress = "n1vuPgRDhX3RoVCc4dbuchZV4EGRyGAKEsx";
...
export default class NebUtils {
   ...
   * 插件模拟执行合约（test）
   */
  static pluginSimCall = (func, args, sucCallback, errCallback) => {
      ...
      listener: (resp) => {
        if (resp.execute_err !== "") {
          if (errCallback) {
            ...
            console.log("pluginSimCall error", resp);
          }
          return;
        }
   ...
   * 轮询交易是否完成
   */
  static intervalQueryTx = (txHash, finishCallback, errCallback) => {
